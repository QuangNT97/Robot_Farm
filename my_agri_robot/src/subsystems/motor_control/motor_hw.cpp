 ...
 *   motor_step  -> PWM consumer for TIM1_CH1 on PA8
 *   motor_dir   -> PE2
 *   motor_ena   -> PE3
 *   motor_alm   -> PE4 (interrupt input, ALM from CL57C)
 */
#define MOTOR_PWM_NODE  DT_NODELABEL(motor_step)
...
static const struct pwm_dt_spec kMotorPwmSpec = PWM_DT_SPEC_GET(MOTOR_PWM_NODE);
...
int MotorHW::SetPulseFrequency(MotorSpeed_t speedHz)
{
    ...
    int ret = pwm_set(m_pwmSpec.dev, m_pwmSpec.channel,
                      periodNs, pulseNs, m_pwmSpec.flags);
    if (ret < 0) {
...
void MotorHW::StopPulse()
{
    ...
    pwm_set(m_pwmSpec.dev, m_pwmSpec.channel,
